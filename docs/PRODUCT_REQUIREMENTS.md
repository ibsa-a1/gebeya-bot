...
>
> **Deferred verification (Phase 12):** the buyer flow (browse → cart → checkout) was fully verified end-to-end in a real desktop browser against the real backend, and the `initData` cryptographic verification logic has full automated test coverage (tampered/wrong-bot/expired signatures correctly rejected, valid signatures correctly accepted). However, **genuinely opening the Mini App inside Telegram's own embedded browser (mobile and desktop) was never successfully confirmed** — repeated attempts using free, account-less `cloudflared` quick tunnels failed with five distinct infrastructure errors (stale DNS, WebSocket incompatibility in dev mode, edge connectivity drops, DNS resolution failures, and one blank-page load with no errors anywhere), none traceable to the application itself. This is a known, explicitly deferred gap — **re-verify the real Telegram embedded-browser experience once a stable, permanent domain exists (Phase 15 deployment)**, before considering this feature launch-ready.
